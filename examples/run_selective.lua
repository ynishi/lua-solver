    ...
    if result.type == "solution" then
        ...
        if result.hypotheses then
            ...
            for i, h in ipairs(result.hypotheses) do
                local eval_mark = h.eval_count > 0
                    and color("0;32", string.format("eval=%d", h.eval_count))
                    or  color("0;31", "not evaluated")
                io.write(string.format("\n  %d. %s\n", i, h.claim))
                io.write(string.format("     %s (%s)  [%s]\n",
                    conf_bar(h.confidence.value), h.confidence.basis, eval_mark))
                dim(string.format("     turn_id=%d  status=%s",
                    h.turn_id or 0, h.status or "?"))
        ...
        if result.eval_result and result.eval_result.evaluated_count then
            ...
            info(string.format("  evaluated: %d / %d hypotheses (budget-limited)",
            ...
            if result.new_hypotheses then
                for _, h in ipairs(result.new_hypotheses) do
                    local status = h.eval_count > 0
                        and color("0;32", "EVALUATED") or color("0;31", "SKIPPED")
        ...
        if result.continuation then
            ...
            if cont.recommend then
                info(string.format("Recommend: another round (expected improvement: +%.2f)",
                    cont.expected_improvement))
                if cont.suggested_action ~= "" then
                    info("  -> " .. cont.suggested_action)
                end
            else
                dim(string.format("Sufficient (expected improvement: +%.2f < threshold)",
                    cont.expected_improvement))
            end
...
dim(string.format("LLM calls: %d (saved by budget: hypotheses=%d, evaluated=%d/turn)",
    Solver.llm.call_count, #problem.hypotheses, 3))
...
for _, h in ipairs(problem.hypotheses) do
    ...
    io.write(string.format("  [%s] eval=%d  conf=%.2f  %s\n",
