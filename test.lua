...
h:add_evidence(S.Evidence({
    content = "b",
    supports = true,
    confidence = S.Confidence({ value = 0.7 }),
    independence_group = "grp-A",
}))
h:add_evidence(S.Evidence({
    content = "c",
    supports = true,
    confidence = S.Confidence({ value = 0.6 }),
    independence_group = "grp-A",
}))
h:add_evidence(S.Evidence({
    content = "d",
...
local active_claims = {}
for _, ah in ipairs(active) do
    active_claims[ah.claim] = true
end
...
local delta_result = strat.re_evaluate.DeltaEval.re_evaluate(
    p_re,
    { low_confidence_bound = 0.5, same_group_weight = 0.3 },
    { "tech" }
)
...
local decay_result = strat.re_evaluate.DecayBased.re_evaluate(
    p_decay,
    { hypothesis_decay_rate = 0.9, supersede_threshold = 0.2 },
    {}
)
...
check(
    "IndependenceWeighted has evaluate_batch",
    type(strat.evidence_eval.IndependenceWeighted.evaluate_batch) == "function"
)
check("SimpleCount has evaluate_batch", type(strat.evidence_eval.SimpleCount.evaluate_batch) == "function")
check("LLM has evaluate_batch", type(strat.evidence_eval.LLM.evaluate_batch) == "function")
...
check("engine default re_evaluate = DeltaEval", eng.strategies.re_evaluate == strat.re_evaluate.DeltaEval)

local eng_noop = engine.new({ re_evaluate = strat.re_evaluate.NoOp })
check("engine re_evaluate swap to NoOp", eng_noop.strategies.re_evaluate == strat.re_evaluate.NoOp)

local eng_decay = engine.new({ re_evaluate = strat.re_evaluate.DecayBased })
check("engine re_evaluate swap to DecayBased", eng_decay.strategies.re_evaluate == strat.re_evaluate.DecayBased)
...
strat.re_evaluate.DecayBased.re_evaluate(p_basis, { hypothesis_decay_rate = 0.9, supersede_threshold = 0.2 }, {})
...
check("UCB1 rank: few-eval hypothesis gets exploration bonus", ucb_ranked[1] == h_ucb_few)
...
local bp_a, bp_b =
    sel_th._beta_params(S.Hypothesis({ claim = "bp", eval_count = 3, confidence = S.Confidence({ value = 0.8 }) }))
...
local sum = 0
for _ = 1, 100 do
    sum = sum + sel_th._sample_beta(10, 2)
end
...
mock_eval_calls = 0
local sel_instance = strat.evidence_eval.Selective(mock_evaluator, strat.hypothesis_selection.UCB1)
local sel_result = sel_instance.evaluate_batch(
    { h_s1, h_s2, h_s3, h_s4 },
    p_sel,
    { eval_budget = 2, exploration_constant = 1.41 }
)
...
mock_eval_calls = 0
local sel_all = strat.evidence_eval.Selective(mock_evaluator, strat.hypothesis_selection.Greedy)
local sel_all_result = sel_all.evaluate_batch({ h_s5, h_s6 }, p_sel, { exploration_constant = 1.41 })
...
check("engine has hypothesis_selection", eng_sel.strategies.hypothesis_selection == strat.hypothesis_selection.UCB1)
